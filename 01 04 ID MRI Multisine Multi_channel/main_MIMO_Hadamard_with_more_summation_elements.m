% Here I try to consider more number of summation elemnts on multi-sine 
clc
clear
close all
...
nMS = nFFT*ind_nMS;
Amp = 1/sqrt(nMS);
frequency_vector_MS = df/ind_nMS:df/ind_nMS:fmax;
for i = 1:1:nMS
    phi_k = -i*(i-1)*pi/nMS;
%     phi_k = x(i);
    inp_dum = inp_dum + Amp*cos(2*pi*frequency_vector_MS(i)*time_vector + phi_k*0);
end
inp_SISO = inp_dum;
figure(1);
...
%     if i == 1
% %         inp(:,i) = (ifft(inp_fft_T(:,i),'symmetric'));
%         inp(:,i) = real(ifft(inp_fft_T(:,i)));
%     else
%         inp(:,i) = imag(ifft(inp_fft_T(:,i)));
%     end
end
figure(2)
