...
n_inp = 1; % n_inpmber of input channels
n_out = 6; % n_inpmber of output channels
...
inp = zeros(size(time_vector,2),n_inp);
Amp = 1;
for j = 1:n_inp
    for i = 1:nFFT
        phi_k = -i*(i-1)*pi/nFFT;
        inp(:,j) = inp(:,j) + Amp*cos(2*pi*frequency_vector(i)*time_vector + phi_k*0)';%rand*pi*2)';
%         rand*pi*2
    end
end
...
n_aver = 1; % n_inpmber of averaging
...
total_samples = round(n_aver*nFFT-(n_aver-1)*r_overlap*nFFT); % total n_inpmber of samples
...
% figure(2)
k = 0;
n_inpm_states = sqrt(numel(sample_sys.A));
for i = 1:n_inp
    for j = 1:n_out
        k = k+1;
...
[out_sample,~,~] = lsim(sample_sys,inp,total_time_vector,zeros(n_inpm_states,1));
% figure(3)
