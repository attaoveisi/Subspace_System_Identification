%% ===========================================================================================
% Multi-input Multi-output code based on Algorithm 1 in MCKELVEY 1997  
% in Proc. of DETC'97 ASME. The method is IV. The Script is written by ATTA OVEISI at  
% Ruhr-Uni. Bochum in Germany. Use of this code without permission is NOT allowed! Contact: atta.oveisi@rub.de
% ============================================================================================
...
q = 45; % Sentence after Eq. (7)
alfa = 65;
% q = 31; % Sentence after Eq. (7)
% alfa = 95;
...
% Create U_q_alfa_N by generalizing Eq. (12)
U_q_N_dum = zeros(m*N,N);
for i = 1:N
    U_q_N_dum((i-1)*m+1:i*m,i) = U;
end
U_q_N = 1/sqrt(N)*W_q_N_p*U_q_N_dum;
...
[Q_dum,R_dum] = qr([real(U_q_alfa_N) imag(U_q_alfa_N);real(Y_q_N) imag(Y_q_N)]',0); % !!!!!!!! Wrong
R_dum = R_dum';
R_11 = R_dum(1:m*q,1:m*q);
R_21 = R_dum(m*q+1:m*q+alfa*m,1:m*q);
R_31 = R_dum(m*q+alfa*m+1:end,1:m*q);
R_22 = R_dum(m*q+1:m*q+alfa*m,m*q+1:m*q+alfa*m);
R_32 = R_dum(m*q+alfa*m+1:end,m*q+1:m*q+alfa*m);
R_33 = R_dum(m*q+alfa*m+1:end,m*q+alfa*m+1:end);
...
for i = 1:m
    for j = 1:p
        k = k+1;
        subplot(m,p,k)
        plot(freq_vec,20*log10(abs(squeeze(G(j,i,1:N)))));
        hold on
